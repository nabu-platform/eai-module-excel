...
public class Services {
	...
	public List<Object> toObject(@NotNull @WebParam(name = "typeId") String typeId, @WebParam(name = "workbook") Workbook workbook, @NotNull @WebParam(name = "sheet") String sheetName, @WebParam(name = "useRegexForSheet") Boolean useRegex, @WebParam(name = "fromRow") Integer fromRow, @WebParam(name = "toRow") Integer toRow, @WebParam(name = "columnsToIgnore") List<Integer> columnsToIgnore, @WebParam(name = "rotate") Boolean rotate, @WebParam(name = "includeEmptyResults") Boolean includeEmptyResults) throws IOException {
		...
		for (int row = 0; row < parsed.length; row++) {
			...
			for (int column = 0; column < parsed[row].length; column++) {
				if (columnsToIgnore == null || !columnsToIgnore.contains(column)) {
					...
					newInstance.set(element.getName(), parsed[row][column]);
					if (isEmptyRow && parsed[row][column] != null) {
						isEmptyRow = false;
					}
				}
			}
